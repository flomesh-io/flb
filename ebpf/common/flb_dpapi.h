/*
 *  flb_dpapi.h: FLB DP Application Programming Interface 
 *  Copyright (C) 2022,  Flomesh <www.flomesh.io>
 * 
 *  SPDX-License-Identifier: (GPL-2.0-or-later OR BSD-2-clause) 
 */
#ifndef __FLB_DPAPI_H__
#define __FLB_DPAPI_H__

#define FLB_MGMT_CHANNEL      "flb0"
#define FLB_SECTION_PASS      "xdp_pass"
#define FLB_FP_IMG_DEFAULT    "/opt/flb/flb_xdp_main.o"
#define FLB_FP_IMG_BPF        "/opt/flb/flb_ebpf_main.o"
#define FLB_FP_IMG_BPF_EGR    "/opt/flb/flb_ebpf_emain.o"
#define FLB_DB_MAP_PDIR       "/sys/fs/bpf"

#define FLB_MAX_LB_NODES      (2)
#define FLB_MIRR_MAP_ENTRIES  (32)
#define FLB_NH_MAP_ENTRIES    (4*1024)
#define FLB_RTV4_MAP_ENTRIES  (32*1024)
#define FLB_RTV4_PREF_LEN     (48)
#define FLB_CT_MAP_ENTRIES    (256*1024*FLB_MAX_LB_NODES)
#define FLB_ACLV6_MAP_ENTRIES (4*1024)
#define FLB_RTV6_MAP_ENTRIES  (2*1024)
#define FLB_TMAC_MAP_ENTRIES  (2*1024)
#define FLB_DMAC_MAP_ENTRIES  (8*1024)
#define FLB_NATV4_MAP_ENTRIES (4*1024)
#define FLB_NATV4_STAT_MAP_ENTRIES (4*16*1024) /* 16 end-points */
#define FLB_SMAC_MAP_ENTRIES  (FLB_DMAC_MAP_ENTRIES)
#define FLB_FW4_MAP_ENTRIES   (8*1024)
#define FLB_INTERFACES        (512)
#define FLB_PORT_NO           (FLB_INTERFACES-1)
#define FLB_PORT_PIDX_START   (FLB_PORT_NO - 128)
#define FLB_INTF_MAP_ENTRIES  (6*1024)
#define FLB_FCV4_MAP_ENTRIES  (FLB_CT_MAP_ENTRIES)
#define FLB_PGM_MAP_ENTRIES   (8)
#define FLB_FCV4_MAP_ACTS     (DP_SET_TOCP)
#define FLB_POL_MAP_ENTRIES   (8*1024)
#define FLB_SESS_MAP_ENTRIES  (20*1024)
#define FLB_PSECS             (8)
#define FLB_MAX_NXFRMS        (16)
#define FLB_CRC32C_ENTRIES    (256)
#define FLB_MAX_MHOSTS        (3)
#define FLB_MAX_MPHOSTS       (7)

#define FLB_DP_SUNP_PGM_ID2    (6)
#define FLB_DP_CRC_PGM_ID2     (5)
#define FLB_DP_CRC_PGM_ID1     (4)
#define FLB_DP_FW_PGM_ID       (3)
#define FLB_DP_CT_PGM_ID       (2)
#define FLB_DP_PKT_SLOW_PGM_ID (1)
#define FLB_DP_PKT_PGM_ID      (0)
...
struct dp_fc_tacts {
  ...
  __u16 pad;
  __u16 pten;
  struct dp_fc_tact fcta[FLB_FCV4_MAP_ACTS];
};
...
typedef struct {
  __u32 nh;
  __be32 mh_host[FLB_MAX_MHOSTS+1];
} ct_sctp_pinfd_t;
...
struct dp_nat_tacts {
  ...
  uint16_t sel_hint;
  uint16_t sel_type;
  uint32_t pmhh[FLB_MAX_MHOSTS];
  struct mf_xfrm_inf nxfrms[FLB_MAX_NXFRMS];
};
...
int flb_fetch_map_stats_cached(int tbl, uint32_t index, int raw, void *bc, void *pc);
void flb_age_map_entries(int tbl);
void flb_collect_map_stats(int tbl);
int flb_fetch_pol_map_stats(int tid, uint32_t e, void *ppass, void *pdrop);
void flb_clear_map_stats(int tbl, __u32 idx);
int flb_add_map_elem(int tbl, void *k, void *v);
int flb_del_map_elem(int tbl, void *k);
void flb_map_loop_and_delete(int tbl, dp_map_walker_t cb, dp_map_ita_t *it);
int flb_dp_link_attach(const char *ifname, const char *psec, int mp_type, int unload);
void flb_xh_lock(void);
void flb_xh_unlock(void);

#endif /* __FLB_DPAPI_H__ */

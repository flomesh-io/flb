/*
 *  flb_kern_fw.c: FLB Kernel eBPF firewall Implementation
...
#define RETURN_TO_MP() bpf_tail_call(ctx, &pgm_tbl, FLB_DP_CT_PGM_ID)
...
dp_do_fw4_main(void *ctx, struct xfi *xf)
{
  ...
  for (i = 0; i < DP_MAX_LOOPS_PER_FWLKUP; i++) {
    ...
    if (!fwe) {
      ...
      /* End of lookup */
      xf->pm.fw_lid = FLB_FW4_MAP_ENTRIES;
      RETURN_TO_MP();
      return DP_DROP;
    } else {
      if (idx == 0) {
  ...
  if (act->ca.act_type == DP_SET_NOP) {
    ...
    FLBS_PPLN_TRAPC(xf, FLB_PIPE_RC_FW_RDR);
  } else {
  ...
  RETURN_TO_MP();
  xf->pm.rcode |= FLB_PIPE_RC_TCALL_ERR;
  return DP_DROP;
}

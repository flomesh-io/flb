/*
 *  flb_kern_sess.c: FLB kernel eBPF Subscriber Session Implementation
...
dp_do_sess4_lkup(void *ctx, struct xfi *xf)
{
  ...
  key.r = 0;
  if (xf->tm.tunnel_id && xf->tm.tun_type != FLB_TUN_IPIP) {

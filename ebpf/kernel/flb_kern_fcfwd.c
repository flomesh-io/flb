/*
 *  flb_kern_fc.c: FLB kernel cache based forwarding
...
dp_do_fcv4_lkup(void *ctx, struct xfi *xf)
{
  ...
  if (acts->fcta[DP_SET_NEIGH_VXLAN].ca.act_type == DP_SET_NEIGH_VXLAN) {
    ...
    dp_do_rt_tun_nh(ctx, xf, FLB_TUN_VXLAN, &ta->ntun);
  }
#endif

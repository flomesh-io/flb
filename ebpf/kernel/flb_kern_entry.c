/*
 *  flb_kern_entry.c: FLB Kernel eBPF entry points
...
#include "flb_kern_cdefs.h"
#include "flb_kern_sum.c"
#include "flb_kern_compose.c"
#include "flb_kern_policer.c"
#include "flb_kern_sessfwd.c"
#include "flb_kern_fw.c"
#include "flb_kern_ct.c"
#include "flb_kern_natlbfwd.c"
#include "flb_kern_l3fwd.c"
#include "flb_kern_l2fwd.c"
#include "flb_kern_devif.c"
#include "flb_kern_fcfwd.c"
...
dp_ing_pkt_main(void *md, struct xfi *xf)
{
  ...
  if (xf->pm.phit & FLB_DP_FC_HIT) {
    dp_parse_d0(md, xf, 0);
  }

  /* Handle parser results */
  if (xf->pm.pipe_act & FLB_PIPE_REWIRE) {
    return dp_rewire_packet(md, xf);
  } else if (xf->pm.pipe_act & FLB_PIPE_RDR) {
    return dp_redir_packet(md, xf);
  }

  if (xf->pm.pipe_act & FLB_PIPE_PASS ||
      xf->pm.pipe_act & FLB_PIPE_TRAP) {
    xf->pm.rcode |= FLB_PIPE_RC_MPT_PASS;
    return DP_PASS;
  }
...
int tc_packet_func_fast(struct __sk_buff *md)
{
  ...
  if (DP_FLB_STAMPED(md) || (md->ingress_ifindex == md->ifindex)) {
    DP_FLB_RST_STAMP(md);
    return DP_PASS;
  } else {
    DP_FLB_STAMP(md);
...
int tc_csum_func1(struct __sk_buff *md)
{
  ...
  if (val == DP_DROP || val == DP_PASS) {
    xf->pm.rcode |= FLB_PIPE_RC_CSUM_DRP;
    TRACER_CALL(md, xf);
  }
  return val;
}
...
int tc_slow_unp_func(struct __sk_buff *md)
{
  ...
  if (val == DP_DROP) {
    xf->pm.rcode |= FLB_PIPE_RC_UNPS_DRP;
    TRACER_CALL(md, xf);
  }
  return val;
}

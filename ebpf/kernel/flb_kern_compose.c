/*
 *  flb_kern_composer.c: FLB Kernel eBPF packet composer/decomposer
             ...
             struct xfi *xf)
{
  ...
  if (tcp->fin)
    tcp_flags = FLB_TCP_FIN;
  if (tcp->rst)
    tcp_flags |= FLB_TCP_RST;
  if (tcp->syn)
    tcp_flags |= FLB_TCP_SYN;
  if (tcp->psh)
    tcp_flags |= FLB_TCP_PSH;
  if (tcp->ack)
    tcp_flags |= FLB_TCP_ACK;
  if (tcp->urg)
    tcp_flags |= FLB_TCP_URG;

  if (p->inp) {
    if (tcp_flags & (FLB_TCP_FIN|FLB_TCP_RST)) {
      xf->pm.il4fin = 1;
    }
...
static int __always_inline
dp_parse_flb(struct parser *p,
             void *md,
             struct xfi *xf)
{
  struct ethhdr *eth;
  struct flb_ethhdr *flb = DP_TC_PTR(p->dbegin);

  LL_DBG_PRINTK("[PRSR] FLB \n");

#ifdef LL_TC_EBPF
  return DP_PRET_FAIL;
#endif

  if (DP_TC_PTR(p->dbegin) + (sizeof(struct ethhdr) + sizeof(*flb)) > p->dend) {
    return DP_PRET_FAIL;
  }

  flb = DP_ADD_PTR(p->dbegin, sizeof(struct ethhdr));
  xf->pm.oport = (flb->oport);
  xf->pm.iport = (flb->iport);

  eth = DP_ADD_PTR(p->dbegin, (int)sizeof(struct flb_ethhdr));
  memcpy(eth->h_dest, xf->l2m.dl_dst, 6);
  memcpy(eth->h_source, xf->l2m.dl_src, 6);
  eth->h_proto = flb->ntype;

  if (dp_remove_l2(md, (int)sizeof(*flb))) {
    return DP_PRET_FAIL;
  }
             ...
             struct xfi *xf)
{
  ...
  if (dp_pkt_is_l2mcbc(xf, md) == 1) {
    FLBS_PPLN_TRAPC(xf, FLB_PIPE_RC_BCMC);
  }
            ...
            int skip_v6)
{
  ...
  } else if (xf->l2m.dl_type == bpf_htons(ETH_TYPE_FLB)) {
    ret = dp_parse_flb(&p, md, xf);
  }
  ...
  if (dp_pkt_is_l2mcbc(xf, md) == 1) {
    FLBS_PPLN_PASSC(xf, FLB_PIPE_RC_BCMC);
  }
...
handle_excp:
  if (ret > DP_PRET_OK) {
    if (ret == DP_PRET_PASS) {
      FLBS_PPLN_PASSC(xf, FLB_PIPE_RC_PARSER);
    } else {
      FLBS_PPLN_TRAPC(xf, FLB_PIPE_RC_PARSER);
    }
  } else if (ret < DP_PRET_OK) {
    FLBS_PPLN_DROPC(xf, FLB_PIPE_RC_PARSER);
  }
  return ret;
}
...
{
  xf->pm.phit |= FLB_DP_UNPS_HIT;

  if (xf->pm.nf & FLB_NAT_SRC) {
    ...
    if (xf->l2m.dl_type == bpf_ntohs(ETH_P_IPV6)) {
      ...
      if (xf->nm.nv6 == 0) {
        ...
        if (xf->pm.pipe_act & (FLB_PIPE_TRAP | FLB_PIPE_PASS)) {
          xf->pm.oport = xf->pm.iport;
          return dp_rewire_port(&tx_intf_map, xf);
        }
      }
    }
  } else if (xf->pm.nf & FLB_NAT_DST) {
...
dp_unparse_packet_always(void *ctx,  struct xfi *xf)
{
  ...
  if (xf->pm.nf & FLB_NAT_SRC && xf->nm.dsr == 0) {
    LL_DBG_PRINTK("[DEPR] LL_SNAT 0x%lx:%x\n",
                 xf->nm.nxip4, xf->nm.nxport);
  ...
  } else if (xf->pm.nf & FLB_NAT_DST && xf->nm.dsr == 0) {
    LL_DBG_PRINTK("[DEPR] LL_DNAT 0x%x\n",
                  xf->nm.nxip4, xf->nm.nxport);
    if (xf->l2m.dl_type == bpf_ntohs(ETH_P_IPV6)) {
      dp_sunp_tcall(ctx, xf);

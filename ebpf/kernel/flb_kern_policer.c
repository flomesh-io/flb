/*
 *  flb_kern_policer.c: FLB eBPF Policer Processing Implementation
...
do_dp_policer(void *ctx, struct xfi *xf, int egr)
{
  ...
  if (pla->pol.color_aware == 0) {
    ...
    if (pla->pol.tok_e < inbytes) {
      xf->qm.ocol = FLB_PIPE_COL_RED;
    } else if (pla->pol.tok_c < inbytes) {
      xf->qm.ocol = FLB_PIPE_COL_YELLOW;
      pla->pol.tok_e -= inbytes;
    } else {
      pla->pol.tok_c -= inbytes;
      pla->pol.tok_e -= inbytes;
      xf->qm.ocol = FLB_PIPE_COL_GREEN;
    }
  } else {
    /* Color-aware mode */
    if (xf->qm.icol == FLB_PIPE_COL_NONE) {
      ret = -1;
      goto out;
    }

    if (xf->qm.icol == FLB_PIPE_COL_RED) {
      xf->qm.ocol = FLB_PIPE_COL_RED;
      goto out;
    }
    ...
    if (pla->pol.tok_e < inbytes) {
      ...
      if (xf->qm.icol == FLB_PIPE_COL_GREEN) {
        xf->qm.ocol = FLB_PIPE_COL_YELLOW;
      } else {
...
out:
  if (pla->pol.drop_prio < xf->qm.ocol) { 
    ...
    FLBS_PPLN_DROPC(xf, FLB_PIPE_RC_POL_DRP);
  } else {

/*
 *  flb_kern_nat.c: FLB Kernel eBPF Stateful NAT/LB Processing
...
dp_sel_nat_ep(void *ctx, struct dp_nat_tacts *act)
{
  ...
  if (act->sel_type == NAT_LB_SEL_RR) {
    ...
    while (n < FLB_MAX_NXFRMS) {
      if (i >= 0 && i < FLB_MAX_NXFRMS) {
...
dp_do_nat(void *ctx, struct xfi *xf)
{
  ...
  if (act->ca.act_type == DP_SET_SNAT || 
    ...
    xf->pm.nf = act->ca.act_type == DP_SET_SNAT ? FLB_NAT_SRC : FLB_NAT_DST;
    xf->nm.npmhh = act->npmhh;
    xf->nm.pmhh[0] = act->pmhh[0];
    xf->nm.pmhh[1] = act->pmhh[1];
    xf->nm.pmhh[2] = act->pmhh[2];  // FLB_MAX_MHOSTS
    ...
    if (sel >= 0 && sel < FLB_MAX_NXFRMS) {
      ...
      if (nxfrm_act < act + 1) {
        ...
        if (xf->l34m.saddr4 == xf->nm.nxip4 && xf->pm.nf == FLB_NAT_DST) {
          xf->nm.nxip4 = 0;
        }
      }
    } else {

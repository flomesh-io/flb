/*
 *  flb_kern_ct.c: FLB kernel eBPF ConnTracking Implementation
             ...
             ct_dir_t dir)
{
  ...
  if (t + 1 > dend) {
    FLBS_PPLN_DROPC(xf, FLB_PIPE_RC_PLCT_ERR);
    return -1;
  }
  ...
  case CT_TCP_CLOSED:
    ...
    if ((tcp_flags & FLB_TCP_SYN) != FLB_TCP_SYN) {
      nstate = CT_TCP_ERR;
      goto end;
    }
  ...
  case CT_TCP_SS:
    ...
    if ((tcp_flags & (FLB_TCP_SYN|FLB_TCP_ACK)) !=
         (FLB_TCP_SYN|FLB_TCP_ACK)) {
      nstate = CT_TCP_ERR;
      goto end;
    }
  ...
  case CT_TCP_SA:
    ...
    if ((tcp_flags & FLB_TCP_ACK) != FLB_TCP_ACK) {
      nstate = CT_TCP_ERR;
      goto end;
    }
  ...
  case CT_TCP_FINI:
    if (ts->fndir != dir) {
      if ((tcp_flags & (FLB_TCP_FIN|FLB_TCP_ACK)) == 
          (FLB_TCP_FIN|FLB_TCP_ACK)) {
        if (ack  != rtd->seq + 1) {
          nstate = CT_TCP_ERR;
          goto end;
        }
    ...
    for (i = 0; i < FLB_MAX_MPHOSTS; i++) {
      if (pm->type == bpf_htons(SCTP_IPV4_ADDR_PARAM)) {
        ...
        if (!atdat->nat_act.nv6) {
          ...
          if (i < FLB_MAX_MHOSTS && atdat->ctd.pi.pmhh[i] != 0) {
    ...
    for (i = 0; i < FLB_MAX_MPHOSTS; i++) {
      if (pm->type == bpf_htons(SCTP_IPV4_ADDR_PARAM)) {
        ...
        if (!axtdat->nat_act.nv6) {
          ...
          if (i < FLB_MAX_MHOSTS && axtdat->ctd.pi.pmhh[i] != 0) {
         ...
         struct dp_ct_tact *axtdat)
{
  ...
  case IPPROTO_SCTP:
    if (tdat->xi.mhon) {
      ...
      for (i = 0; i < pss->nh && i < FLB_MAX_MHOSTS; i++) {
        key->saddr[0] = pss->mh_host[i];
        for (j = 0; j < FLB_MAX_MHOSTS; j++) {
          if (tdat->pi.pmhh[j]) {
...
#if 0
      for (i = 0; i < pxss->nh && i < FLB_MAX_MHOSTS; i++) {
        xkey->saddr[0] = pxss->mh_host[i];
        xkey->saddr[1] = 0;
        xkey->saddr[2] = 0;
        xkey->saddr[3] = 0;
        for (j = 1; j < FLB_MAX_MHOSTS; j++) {
          if (xtdat->pi.pmhh[j]) {
...
dp_ct_in(void *ctx, struct xfi *xf)
{
  ...
  if (xf->pm.nf & (FLB_NAT_DST|FLB_NAT_SRC)) {
    if (DP_XADDR_ISZR(xi->nat_xip)) {
      if (xf->pm.nf == FLB_NAT_DST) {
        xi->nat_flags = FLB_NAT_HDST;
      } else if (xf->pm.nf == FLB_NAT_SRC){
        xi->nat_flags = FLB_NAT_HSRC;
      }
    }
  }
  ...
  if (atdat == NULL || axtdat == NULL) {
    ...
    if (xi->nat_flags) {
      adat->ca.act_type = xi->nat_flags & (FLB_NAT_DST|FLB_NAT_HDST) ?
    ...
    adat->ctd.pi.pmhh[2] = xf->nm.pmhh[2]; // FLB_MAX_MHOSTS
    ...
    if (xxi->nat_flags) { 
      axdat->ca.act_type = xxi->nat_flags & (FLB_NAT_DST|FLB_NAT_HDST) ?
                             DP_SET_DNAT: DP_SET_SNAT;
    ...
    axdat->ctd.pi.pmhh[2] = xf->nm.pmhh[2]; // FLB_MAX_MHOSTS

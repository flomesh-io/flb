/*
 *  flb_kern_l2fwd.c: FLB kernel eBPF L2 forwarder Implementation
...
dp_do_smac_lkup(void *ctx, struct xfi *xf, void *fc)
{
  ...
  if (!sma) {
    /* Default action */
    FLBS_PPLN_PASSC(xf, FLB_PIPE_RC_NOSMAC);
    return 0;
  }

  xf->pm.phit |= FLB_DP_SMAC_HIT;
...
dp_do_dmac_lkup(void *ctx, struct xfi *xf, void *fa_)
{
  ...
  if (!dma) {
    ...
    FLBS_PPLN_PASSC(xf, FLB_PIPE_RC_NODMAC);
    return 0;
  }

  xf->pm.phit |= FLB_DP_DMAC_HIT;
...
dp_do_nh_lkup(void *ctx, struct xfi *xf, void *fa_)
{
  ...
  if (!nha) {
    /* No NH - Trap */
    FLBS_PPLN_TRAPC(xf, FLB_PIPE_RC_ACT_UNK);
    return 0;
  }

  xf->pm.phit |= FLB_DP_NEIGH_HIT;
...
#endif
    return dp_do_rt_tun_nh(ctx, xf, FLB_TUN_VXLAN, &nha->rt_tnh);
  } else if (nha->ca.act_type == DP_SET_NEIGH_IPIP) {
    return dp_do_rt_tun_nh(ctx, xf, FLB_TUN_IPIP, &nha->rt_tnh);
  }

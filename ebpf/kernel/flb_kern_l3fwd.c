/*
 *  flb_kern_l3fwd.c: FLB Kernel eBPF L3 forwarder Implementation
...
dp_do_rt4_fwdops(void *ctx, struct xfi *xf)
{
  ...
  if (iph + 1 > dend)  {
    FLBS_PPLN_DROPC(xf, FLB_PIPE_RC_PLRT_ERR);
    return -1;
  }
  ip_decrease_ttl(iph);
  return 0;
...
dp_rtv4_get_ipkey(struct xfi *xf)
{
  ...
  if (xf->pm.nf & FLB_NAT_DST) {
    ...
    if (xf->pm.nf & FLB_NAT_SRC) {
      ...
      if (xf->tm.new_tunnel_id && xf->tm.tun_type == FLB_TUN_GTP) {
...
dp_do_rtops(void *ctx, struct xfi *xf, void *fa_, struct dp_rt_tact *act)
{
  ...
  if (act->ca.act_type == DP_SET_DROP) {
    ...
    FLBS_PPLN_TRAPC(xf, FLB_PIPE_RC_RT_TRAP);
...
dp_do_rtv6(void *ctx, struct xfi *xf, void *fa_)
{
  ...
  if (!act) {
    ...
    xf->pm.nf &= ~FLB_NAT_SRC;
    LL_DBG_PRINTK("RT Not found");
    return 0;
  }

  xf->pm.phit |= FLB_DP_RT_HIT;
...
{
  xf->pm.nf = do_snat ? FLB_NAT_SRC : FLB_NAT_DST;
             ...
             struct dp_ct_tact *act)
{
  ...
  if (act->ca.act_type == DP_SET_DO_CT) {
    ...
    /*FLBS_PPLN_TRAP(xf);*/
    FLBS_PPLN_TRAPC(xf, FLB_PIPE_RC_ACL_TRAP);
...
ct_trk:
  return dp_tail_call(ctx, xf, fa_, FLB_DP_CT_PGM_ID);
}
...
dp_ing_l3(void *ctx,  struct xfi *xf, void *fa)
{
  if (xf->l2m.dl_type == bpf_htons(ETH_P_IP)) {
    /* Check termination */
    if (xf->tm.tunnel_id &&
        (xf->tm.tun_type == FLB_TUN_GTP || xf->tm.tun_type == FLB_TUN_IPIP)) {
      dp_do_sess4_lkup(ctx, xf);
    }
  }

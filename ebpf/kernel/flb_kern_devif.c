/*
 *  flb_kernel_devif.c: FLB kernel eBPF dev in/out pipeline
...
dp_do_if_lkup(void *ctx, struct xfi *xf)
{
  ...
  if (!l2a) {
    ...
    FLBS_PPLN_PASSC(xf, FLB_PIPE_RC_UNX_DRP);
    return -1;
  }

  xf->pm.phit |= FLB_DP_IF_HIT;
  LL_DBG_PRINTK("[INTF] L2 action %d\n", l2a->ca.act_type);

  if (l2a->ca.act_type == DP_SET_DROP) {
    FLBS_PPLN_DROPC(xf, FLB_PIPE_RC_ACT_DROP);
  } else if (l2a->ca.act_type == DP_SET_TOCP) {
    FLBS_PPLN_TRAPC(xf, FLB_PIPE_RC_ACT_TRAP);
    ...
    FLBS_PPLN_DROPC(xf, FLB_PIPE_RC_ACT_UNK);
  }
...
dp_do_mirr_lkup(void *ctx, struct xfi *xf)
{
  ...
  if (!ma) {
    FLBS_PPLN_DROPC(xf, FLB_PIPE_RC_UNX_DRP);
    return -1;
  }
...
dp_trap_packet(void *ctx,  struct xfi *xf, void *fa_)
{
  ...
  if (dp_add_l2(ctx, (int)sizeof(*flb))) {
  ...
  neth->h_proto = bpf_htons(ETH_TYPE_FLB); 
  
  /* Add FLB shim */
  flb = DP_ADD_PTR(neth, sizeof(*neth));
  if (flb + 1 > dend) {
    return DP_DROP;
  }

  flb->iport = bpf_htons(xf->pm.iport);
  flb->oport = bpf_htons(xf->pm.oport);
  flb->rcode = xf->pm.rcode;
  if (xf->tm.tun_decap) {
    flb->rcode |= FLB_PIPE_RC_TUN_DECAP;
  }
  flb->mmap = xf->pm.table_id; /* FIXME */
  flb->ntype = ntype;

  xf->pm.oport = FLB_PORT_NO;
...
dp_pipe_check_res(void *ctx, struct xfi *xf, void *fa)
{
  ...
  if (xf->pm.pipe_act) {
    ...
    if (DP_FLB_IS_EGR(ctx)) {
      if (xf->pm.nf == 0) {
        return DP_PASS;
      }
    }

    if (xf->pm.pipe_act & FLB_PIPE_DROP) {
      return DP_DROP;
    } 
...
#ifndef HAVE_FLB_DISAGGR
#ifdef HAVE_OOB_CH
    if (xf->pm.pipe_act & FLB_PIPE_TRAP) { 
      return dp_trap_packet(ctx, xf, fa);
    } 

    if (xf->pm.pipe_act & FLB_PIPE_PASS) {
#else
    ...
    if (xf->pm.pipe_act & FLB_PIPE_RDR_MASK) {
...
dp_ing_ct_main(void *ctx,  struct xfi *xf)
{
   ...
   */
  if ((xf->pm.phit & FLB_DP_CTM_HIT) == 0) {

    if (xf->pm.fw_lid < FLB_FW4_MAP_ENTRIES) {
      bpf_tail_call(ctx, &pgm_tbl, FLB_DP_FW_PGM_ID);
      return DP_PASS;
    }

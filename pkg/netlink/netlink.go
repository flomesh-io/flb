...
const (
	...
	CertPath = "/opt/fsmxlb/cert/"

	// CACertFileName - fsmxlb CA cert file
	CACertFileName = "rootCA.crt"

	// PrivateCertName - fsmxlb private certificate name
	PrivateCertName = "server.crt"

	// PrivateKeyName - fsmxlb private key name
	PrivateKeyName = "server.key"
)

const (
	// AddrUpdateWorkqLen - Address worker channel depth
	AddrUpdateWorkqLen = 1024
	// LinkUpdateWorkQLen - Link worker channel depth
	LinkUpdateWorkQLen = 1024
	// NeighUpdateWorkQLen - Neigh worker channel depth
	NeighUpdateWorkQLen = 1024
	// RouteUpdateWorkQLen - Route worker channel depth
	RouteUpdateWorkQLen = 40827
)
...
// NetHookInterface - Go interface which needs to be implemented to talk to xlbnet module

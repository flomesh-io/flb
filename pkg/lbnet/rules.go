...
func RulesInit(zone *Zone) *RuleH {
	...
	if certExists == true && keyExists == true {
		...
		if err != nil {
			tk.LogIt(tk.LogError, "Error loading flb certificate %s and key file %s",
				certFile, keyFile)
		}
		nRh.tlsCert = cert
	}
...
func (R *RuleH) AddNatLbRule(serv cmn.LbServiceArg, servSecIPs []cmn.LbSecIPArg, servEndPoints []cmn.LbEndPointArg) (int, error) {
	...
	r.secIP = nSecIP
	// Per LB end-point health-check is supposed to be handled at kube-flb/CCM,
	// but it certain cases like stand-alone mode, flb can do its own
...
func (R *RuleH) DeleteNatLbRule(serv cmn.LbServiceArg) (int, error) {
	...
	if R.vipMap[sNetAddr.IP.String()] == 0 {
		if IsIPHostAddr(sNetAddr.IP.String()) {
			nlp.DelAddrNoHook(sNetAddr.IP.String()+"/32", "lo")
		}
...
func (R *RuleH) AdvRuleVIPIfL2(IP net.IP) error {
	...
	//			if nlp.AddAddrNoHook(IP.String()+"/32", "lo") != 0 {
	...
	//	if IsIPHostAddr(IP.String()) {
	//		if nlp.DelAddrNoHook(IP.String()+"/32", "lo") != 0 {
